...
bool is_adjacent_to_symbol(char (*grid)[141], int row, int col) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (is_valid(adj_row, adj_col) && ispunct(adj_symbol) && adj_symbol != '.') {
            return true;
        }
    }
    return false;
...
int calculate_gear(char (*grid)[141], int row, int col) {
        ...
        for (int j = 0; j < 141; j++) {
            if (isdigit(grid[i][j]) && is_valid_part_number(grid, i, j)) {
                ...
                while (isdigit(grid[i][j])) {
                    part_number = add_digit(part_number, grid[i][j] - '0');
                    j++;
                }
...
void part2_script() {
    ...
    for (int i = 0; i < 140; i++) {
        for (int j = 0; j < 141; j++) {
            if (grid[i][j] == '*') {
                int gear = calculate_gear(grid, i, j);
